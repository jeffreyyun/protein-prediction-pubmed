...
def process_check_point():
	try:
		...
		return 0
	global check_point
	if os.path.isfile(dir_name+"/check_point.txt"):
		check_point_file = open(dir_name+"/check_point.txt", "r")
		check_point= int(check_point_file.readline())
		check_point_file.close()
		return check_point
	else:
		check_point_file = open(dir_name+"/check_point.txt", "w")
...
def fetch_data(webenv, query_key, id_list, checkpoint):
	attempt = 0
    filename = dir_name+"/data"+str(checkpoint)+".fasta";
	while attempt < 3:
		attempt += 1
		failure = 0
		try:
