...
def CheckURI(uri, timeout=5):
    try:
        ...
        return (r.status_code and r.status_code == 200)
    except:
        return False
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if shortened_name_exists(name):
                self.send_response(303)
                self.send_header('Location', get_original_name(name))
                self.end_headers()
            else:
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Please fill out both!")
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("I don't know '{}'.".format(longuri).encode())
